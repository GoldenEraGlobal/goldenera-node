...
ge_calculate_auto_heap_mb 15990 11000 4000 4000 true FULL 512 32 2 true 512 true
assert_eq "$GE_AUTO_HEAP_MB" 3072 "16GB VM with an oversized legacy huge-page pool"
...
ge_calculate_auto_heap_mb 32768 28500 2560 2560 true FULL 512 32 2 true 512 true
assert_eq "$GE_AUTO_HEAP_MB" 16384 "32GB full-memory mining VM"

ge_calculate_auto_heap_mb 8192 8192 0 0 false FULL 512 32 2 true 512 false
assert_eq "$GE_AUTO_HEAP_MB" 2304 "non-mining Explorer container with an 8GB cgroup budget"

if ge_calculate_auto_heap_mb 4096 3000 2560 2560 true FULL 512 32 2 true 512 true; then
  fail "insufficient-memory configuration unexpectedly produced a heap"
fi

ge_calculate_auto_heap_mb 15990 12500 2560 2560 true FULL 512 32 2 true 512 false
assert_eq "$GE_AUTO_HEAP_MB" 2048 "large-page fallback remains inside a 16GB budget"
...
printf '%s\n' '1073741824' >"$fixture_dir/memory.current"

GOLDENERA_MEMINFO_PATH="$fixture_dir/meminfo" \
GOLDENERA_CGROUP_V2_MEMORY_MAX_PATH="$fixture_dir/memory.max" \
GOLDENERA_CGROUP_V2_MEMORY_CURRENT_PATH="$fixture_dir/memory.current" \
GOLDENERA_CGROUP_V1_MEMORY_LIMIT_PATH="$fixture_dir/missing-v1" \
	GOLDENERA_CGROUP_V1_MEMORY_USAGE_PATH="$fixture_dir/missing-usage-v1" \
...
assert_eq "$GE_CGROUP_MEMORY_USAGE_MB" 1024 "cgroup v2 usage detection"
assert_eq "$GE_MEMORY_AVAILABLE_MB" 7168 "available memory uses cgroup headroom"
...
GOLDENERA_CGROUP_V2_MEMORY_CURRENT_PATH="$fixture_dir/missing-current-v2" \
